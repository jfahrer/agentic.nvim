...
- [x] 3.5 RED: Add integration tests that failed tool calls remain open
  by default
- [x] 3.6 GREEN: Keep failed tool calls foldable but open by default
- [x] 3.7 GREEN: Route live tool-call writes through `MessageWriter`
  so replay and live updates share the same fold-sync path
...
- [x] 4.8 GREEN: Capture fold state on widget hide/show callbacks so
  reopened windows preserve manual open/closed choices
- [x] 4.9 GREEN: Reset fold bookkeeping when a session is cleared or a
  restore conflict replaces the current session
- [x] 4.10 GREEN: Clear the restoring flag if restore-time session
  creation fails
...
- [x] 5.4 Run `make benchmark-tool-call-folding` and confirm the hot path,
  hidden backfill, reopen, and initial creation benchmarks stay fast
